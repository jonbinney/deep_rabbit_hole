...
from quoridor import Player, array_to_action, construct_game_from_observation
...
@dataclass
class SimpleDexpParams(SubargsBase):
...
@njit(cache=True)
def sample_actions(
    grid, dqn_agent, player_positions, walls_remaining, goal_rows, current_player, branching_factor, wall_sigma
):
    """
...
class SimpleDexpAgent(Agent):
    def __init__(self, params=SimpleDexpParams(), **kwargs):
        super().__init__()
        self.params = params
        self.board_size = kwargs["board_size"]
        self.dqn_agent = AgentRegistry.create_from_encoded_name(
            urllib.parse.unquote(params.encoded_dqn_agent_params), **kwargs
        )

    @classmethod
    def params_class(cls):
        return SimpleDexpParams
    ...
    def name(self):
        ...
        return "simple_dexp" + ",".join(param_strings)
