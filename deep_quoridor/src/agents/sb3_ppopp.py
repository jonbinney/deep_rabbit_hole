###
# PPO from Stable Baslines 3, trained using chosen opponents intstead of against itself.
###
...
class SB3ActionMaskWrapper(BaseWrapper):
    ...
    def __init__(self, env, rewards_multiplier: float = 1000, opponent: Agent = None):
        super().__init__(env)
    ...
    def step(self, opponent_action):
        ...
        if not truncation and not termination and self.opponent is not None:
            ...
            if opponent_agent == "player_1" and raw_opponent_action is not None:
                ...
                from agents.core.rotation import convert_rotated_action_index_to_original
                opponent_action = convert_rotated_action_index_to_original(unwrapped_env.board_size, raw_opponent_action)
            else:
